...
void IoT_device::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet" ) { // the device receives a packet from the sink
        ...
        // if dst == itself
        if (dst_id == this_id) {
            // update parent / child_ct (no change now)
            // parent = l3->getParent();
...
void IoT_sink::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet" ) { // the device receives a packet from the sink
        ...
        // int child_parent array
        child_parent = new unsigned int[Nodes];
        ...
        // (store nodes parent)
        child_parent[src_id] = l3->getParent();
        ...
        // set new parent / children / child_ct
